...
static int drv_sensor_current_dev_init(struct drv_curr_sensor_clx *curr)
{
    return DRIVER_OK;
}

int drv_sensor_current_init(void **current_driver)
{
    struct drv_curr_sensor_clx *curr = &drv_sensor;
    int ret;

    LOG_INFO(CLX_DRIVER_TYPES_CURR, "clx_driver_clx8000_current_init\n");
    ret = drv_sensor_current_dev_init(curr);
    if (ret != DRIVER_OK)
        return ret;

    curr->current_if.get_main_board_curr_number = drv_sensor_get_main_board_curr_number;
    curr->current_if.get_main_board_curr_alias = drv_sensor_get_main_board_curr_alias;
    curr->current_if.get_main_board_curr_type = drv_sensor_get_main_board_curr_type;
    curr->current_if.get_main_board_curr_max = drv_sensor_get_main_board_curr_max;
    curr->current_if.set_main_board_curr_max = drv_sensor_set_main_board_curr_max;
