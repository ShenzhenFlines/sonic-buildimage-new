...
static unsigned short process_power(struct i2c_client *client, int page, int reg)
{
    ...
    mfr_sys_cfg = pmbus_read_word_data(client, 0, MP2888_MFR_SYS_CONFIG);
...
static unsigned int process_iout(struct i2c_client *client, int page, int reg)
{
    ...
    mfr_sys_cfg = pmbus_read_word_data(client, page, MP2888_MFR_SYS_CONFIG);
    resloution = (mfr_sys_cfg >> 3) & 0x1;
...
static int mp2886_read_word_data(struct i2c_client *client, int page, int reg)
{
    switch (reg) {
...
#if 0
static int
mp2886_current_sense_gain_and_resolution_get(struct i2c_client *client, struct mp2886_data *data)
{
    int ret;
...
static int
mp2886_read_phase(struct i2c_client *client, struct mp2886_data *data, int page, int phase, u8 reg)
{
    int ret;
...
static int
mp2886_read_phases(struct i2c_client *client, struct mp2886_data *data, int page, int phase)
{
    int ret;

    switch (phase) {
    case 0 ... 1:
        ret = mp2886_read_phase(client, data, page, phase, MP2888_MFR_READ_CS1_2);
        break;
    case 2 ... 3:
        ret = mp2886_read_phase(client, data, page, phase, MP2888_MFR_READ_CS3_4);
        break;
    case 4 ... 5:
        ret = mp2886_read_phase(client, data, page, phase, MP2888_MFR_READ_CS5_6);
        break;
    case 6 ... 7:
        ret = mp2886_read_phase(client, data, page, phase, MP2888_MFR_READ_CS7_8);
        break;
    case 8 ... 9:
        ret = mp2886_read_phase(client, data, page, phase, MP2888_MFR_READ_CS9_10);
        break;
    default:
        return -ENODATA;
    }
...
static int mp2886_read_word_data_mp2888(struct i2c_client *client, int page, int phase, int reg)
{
    const struct pmbus_driver_info *info = pmbus_get_driver_info(client);
    struct mp2886_data *data = to_mp2886_data(info);
    int ret;
    ...
    case PMBUS_READ_IOUT:
        if (phase != 0xff)
            return mp2886_read_phases(client, data, page, phase);
...
static int mp2886_write_word_data(struct i2c_client *client, int page, int reg, u16 word)
{
    const struct pmbus_driver_info *info = pmbus_get_driver_info(client);
    struct mp2886_data *data = to_mp2886_data(info);
...
static int
mp2886_identify_multiphase(struct i2c_client *client, struct mp2886_data *data,
               struct pmbus_driver_info *info)
{
    int ret;
