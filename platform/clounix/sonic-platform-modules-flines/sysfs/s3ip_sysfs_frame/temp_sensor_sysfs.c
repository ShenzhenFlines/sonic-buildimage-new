...
static ssize_t sensor_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;
...
static ssize_t sensor_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;
...
static ssize_t temp_sensor_max_hyst_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int temp_index;
    int ret;

    check_p(g_temp_sensor_drv);
    check_p(g_temp_sensor_drv->get_main_board_temp_max_hyst);
                                          ...
                                          const char *buf, size_t count)
{
    ...
    check_p(g_temp_sensor_drv);
    check_p(g_temp_sensor_drv->set_main_board_temp_max_hyst);
...
static struct switch_attribute num_temp_attr = __ATTR(num_temp_sensors, S_IRUGO, temp_sensor_number_show, NULL);
// share debug for sensors
static struct switch_attribute sensor_debug_attr = __ATTR(debug, S_IRUGO | S_IWUSR, sensor_debug_show, sensor_debug_store);
static struct switch_attribute sensor_loglevel_attr = __ATTR(loglevel, S_IRUGO | S_IWUSR, sensor_loglevel_show, sensor_loglevel_store);
